...
- `WARM-006`: After bootstrap, or from the first request when bootstrap is disabled, every configurable model-input family in the main agent's provider request equals the configured `parent-only` plus `shared` assignments and includes the configured short guidance. The child request equals `child-only` plus `shared` assignments. This covers system-prompt sections, runtime contexts, durable AGENTS input, model-visible skill input, and tool schemas; a source assigned to one role does not enter the other role's request.
...
- `WARM-008`: Every known DSH prompt/context and tool source appears with a readable contribution name, tool description where applicable, and an explicit package default based on responsibility. Genuinely unknown sources also appear and default to `child-only`. Stable identity, not discovery order or a same-name guess, determines saved assignments across reload.
...
- `WARM-010`: The official Plugins settings page exposes a compact summary and opens the complete editor in a wide package-owned modal. Prompt/context and tool lists collapse independently. Each source uses a three-slot single-choice control for main only, child only, or shared visibility; tool rows show names and description previews while opaque source IDs remain secondary details. Saves are validated and persist through the Host settings provider without overwriting concurrent edits.
...
- Plugin revision `4c4344d` is the current implementation evidence for the self-contained roster, configurable role projection, readable inventory Remote and Plugins manager, and exact Host patch lifecycle. Documentation and intent records may follow it without changing runtime semantics.
